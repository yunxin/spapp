    ...
    renderViewBody: function SP_UI_Blogs_BlogRenderer$renderViewBody($p0) {
        if (this.baseViewID === 8) {
            ...
            if (!SP.ScriptHelpers.isNullOrUndefinedOrEmpty($v_3)) {
                ...
                $p0.ListData['PrevHref'] = SP.ScriptHelpers.replaceOrAddQueryString($v_3, 'Name', $v_2);
            }
            if (!SP.ScriptHelpers.isNullOrUndefinedOrEmpty($v_4)) {
                $v_4 = SP.ScriptHelpers.replaceOrAddQueryString($v_4, 'CategoryId', $v_1);
                $p0.ListData['NextHref'] = SP.ScriptHelpers.replaceOrAddQueryString($v_4, 'Name', $v_2);
            }
        }
        else if (this.baseViewID === 9) {
            var $v_5 = SP.ScriptHelpers.getDocumentQueryPairs();
            var $v_6 = $v_5['StartDateTime'];
            var $v_7 = $v_5['EndDateTime'];
            var $v_8 = $v_5['LMY'];
            var $v_9 = $p0.ListData['PrevHref'];
            var $v_A = $p0.ListData['NextHref'];

            if (!SP.ScriptHelpers.isNullOrUndefinedOrEmpty($v_9)) {
                $v_9 = SP.ScriptHelpers.replaceOrAddQueryString($v_9, 'StartDateTime', $v_6);
                $v_9 = SP.ScriptHelpers.replaceOrAddQueryString($v_9, 'EndDateTime', $v_7);
                $p0.ListData['PrevHref'] = SP.ScriptHelpers.replaceOrAddQueryString($v_9, 'LMY', $v_8);
            }
            if (!SP.ScriptHelpers.isNullOrUndefinedOrEmpty($v_A)) {
                $v_A = SP.ScriptHelpers.replaceOrAddQueryString($v_A, 'StartDateTime', $v_6);
                $v_A = SP.ScriptHelpers.replaceOrAddQueryString($v_A, 'EndDateTime', $v_7);
                $p0.ListData['NextHref'] = SP.ScriptHelpers.replaceOrAddQueryString($v_A, 'Name', $v_8);
            }
        }
    ...
    case 303:
        for (var $v_A = 0; $v_A < $v_1.length; $v_A++) {
            ...
            case 'Name':
                ...
                $v_C = ($v_C.replace(new RegExp('&nbsp;', 'g'), ' ')).replace(new RegExp('&#160;', 'g'), ' ');
                var $v_D = new RegExp('[\"#%&*:<>?\\\\/{|}~. ]', 'g');
                var $v_E = $v_C.replace($v_D, '-');

                $p0 = $p0.replace($v_1[$v_A], $v_E);
                break;
            case 'CategoryId':
                $v_B = 'ID';
