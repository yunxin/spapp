function $_global_sp_ui_listsearchboxbootstrap() {
    ...
    (function() {
        ...
        var InsertSearchBox = function(renderCtx, specificListSearchBoxInfo) {
            var elem = document.getElementById('inplaceSearchDiv_' + ctx.wpq);

            if (elem == null) {
                var parentDiv = document.getElementById('CSRListViewControlDiv' + ctx.wpq);

                if (parentDiv !== null) {
                    var newDiv = document.createElement('div');

                    newDiv.id = 'inplaceSearchDiv_' + ctx.wpq;
                    newDiv.className = 'ms-InlineSearch-DivBaseline';
                    var saveNode = parentDiv.firstChild;

                    if (saveNode != null && saveNode.id == ctx.wpq + '_ListTitleViewSelectorMenu_Container') {
                ...
                if (typeof elem.control == 'undefined') {
                    ...
                    var SodLoaded = function() {
                        ...
                        if (typeof specificListSearchBoxInfo.searchBoxConstructor === 'function') {
                            specificListSearchBox = specificListSearchBoxInfo.searchBoxConstructor(ctx, specificListSearchBoxInfo.elem);
                        }
                        else {
                            specificListSearchBox = new Microsoft.SharePoint.Portal.ListSearchBox(ctx, specificListSearchBoxInfo.elem);
                        }
                    ...
                    var ShowSodLoadingUI = function() {
                        var loadingImageUrl = ctx.imagesPath + "loadingcirclests16.gif";
